...
public class DataSourceTest {
	...
	public String memberPrimaryKey() {
		//사용자 프라이머리 생성하는 메서드 년월일시분초+밀리 초
	...
	public void updateMember() throws Exception {
		//CRUD 중 Update 테스트 구현 /특징: user_id는 프라이머리키 이기 때문에 수정대상이 아님.
		//MemberVO memberVO = new MemberVO();  @Inject로 했기 때문에 주석처리함.	
		memberVO.setUser_id("admin");
		memberVO.setUser_name("홍길동");
		memberVO.setUser_pw(""); //암호를 수정하기 원하지 않는 사람을 가정
		memberVO.setEmail("test@test.com");
		memberVO.setPoint(100);
		memberVO.setEnabled(true);
		memberVO.setLevels("ROLE_ADMIN");
		String user_id = memberVO.getUser_id(); //(그냥 참고용 코드해 놓음)memberVO의 오브젝트의 데이터는 1개의 레코드이기 때문에 반환 값이 1개만.
	...
	public void readMember() throws Exception {
		...
		System.out.println("admin에 대한 상세정보");
		System.out.println(memberVO.toString());
	}
	
	@Test
	public void deleteMember() throws Exception {
		//CRUD 중 Delete 테스트(쿼리->(IF_)DAO->(Impl)memberDAO주입받은 오브젝트 사용)
		memberDAO.deleteMember("user_20201215145804115");
	}
	...
	public void insertMember() throws Exception {
		//CRUD 중 Create 테스트
		//MemberVO memberVO = new MemberVO(); @Inject로 했기 때문에 주석처리함.
		//사용자 생성 규칙: user_로 시작(prefix),suffix(접미사)는 년월일시분초 
		...
		//패스워드 암호화 처리 (필수이지만, 스프링 시큐리티 할 때 처리예정)
		memberVO.setUser_pw("1234");
		memberVO.setEmail("asc@aka.com");
		memberVO.setPoint(100);
		memberVO.setEnabled(true);
	...
	public void dbConnectTest() {
		try {//내부에서 {} 에러발생 시 실행을 중지하고, catch{}구문이 실행됨. 예외처리라고 함
			Connection connection = dataSource.getConnection();
			System.out.println("데이터베이스 접속이 성공하였습니다.");
		} catch (SQLException e) {
			System.out.println("데이터베이스 접속에 실패하였습니다. 왜냐하면 " + e);
			//e.printStackTrace();
		}//DB커넥션 오브젝트 만드는 과정. new사용 안함.
		//try~catch는 테스트에서만 사용하고, 스프링에서는 throws Exception으로 예외를 표시함.
